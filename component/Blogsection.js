    ...
    <div className="flex flex-col gap-5 mx-auto max-w-3xl lg:max-w-4xl">
      {posts.map((post, index) => (
        <Link key={index} href={`/blog/${post.slug.current}`} passHref>
          <div className="flex border border-gray-200 rounded-lg overflow-hidden items-center transform transition-all hover:shadow-md hover:border-amber-300 cursor-pointer">
            <img className=" w-32 lg:w-6/12  object-contain" src={urlFor(post.mainImage).url()} alt={post.title} />
